...
\newcommand{\MMEntityReference}[3]{%
  \@ifundefined{mm@#1@#2}{%
    \MMNamedEntity{#1}{#2}%
  }{%
    \hyperref[#1:#2]{#3{\MMNamedEntity{#1}{#2}}}%
  }%
}
\makeatother
...
% Every entity reference links to the matching sheet or mystery section.
\newcommand{\Character}[1]{\MMEntityReference{character}{#1}{\CharacterReferenceStyle}}
\newcommand{\Space}[1]{\MMEntityReference{space}{#1}{\SpaceReferenceStyle}}
\newcommand{\Clue}[1]{\MMEntityReference{clue}{#1}{\ClueReferenceStyle}}
\newcommand{\Mystery}[1]{\MysteryReferenceStyle{\nameref{mystery:#1}}}

% Consistent inline formatting for times and entities without their own sheet.
\newcommand{\timestamp}[1]{\TimestampStyle{#1}}
\newcommand{\outsideentity}[1]{\OutsideEntityStyle{#1}}
...
% registration pass it records the ID and display name; during normal rendering it typesets the body.
\newcommand{\CharacterSheet}[4]{%
  \ifMMRegistrationPass
    ...
    \fi
  \else
    \subsubsection{#2: \CharacterReferenceStyle{\CharacterName{#1}}}
    \label{character:#1}%
    #4%
  \fi
}
...
\newcommand{\SpaceSheet}[3]{%
  ...
  \else
    \subsubsection{\SpaceReferenceStyle{\SpaceName{#1}}}
    \label{space:#1}%
    #3%
  \fi
}
...
\newcommand{\ClueSheet}[3]{%
  ...
  \else
    \subsubsection{\ClueReferenceStyle{\ClueName{#1}}}
    \label{clue:#1}%
    #3%
  \fi
}

\newcommand{\MysterySheet}[3]{%
  \ifMMRegistrationPass
...
\newcommand{\RelationshipSummary}{%
  ...
  There are \arabic{mysteryCount} mysteries.\par
}
